...
<body>
    ...
    <div id="sidebar">
      <div class="header">
        ...
        <div style="display:flex;align-items:center;gap:8px;">
          <a href="./architecture/" style="text-decoration:none;font-size:0.75rem;font-weight:700;padding:4px 8px;border-radius:6px;background:rgba(201,168,76,0.18);border:1px solid #c9a84c;color:#e5c76b;">Architecture ➔</a>
  ...
  <script type="module">
    ...
    function startCosmicOrbit() {
      if (orbitAnimFrame) {
        window.cancelAnimationFrame(orbitAnimFrame);
        orbitAnimFrame = null;
      }
    ...
    function updateGraphVisibility() {
      ...
      visNodes.forEach(node => {
        nodeUpdates.push({ id: node.id, hidden: !visibleNodeIds.has(node.id) });
      });
    ...
    function updateNodeScale() {
      const updates = [];
      visNodes.forEach(node => updates.push({ id: node.id, size: Math.round(node.baseSize * uiState.nodeScale) }));
      visNodes.update(updates);
    }

    function updateLabelScale() {
      const updates = [];
      visNodes.forEach(node => updates.push({
        id: node.id,
