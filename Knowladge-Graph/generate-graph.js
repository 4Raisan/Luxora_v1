...
<body>
    ...
    <details class="settings-dock" open>
      ...
      <div class="settings-content">
        <div class="settings-section">
          ...
          <label class="settings-row"><span>Animate graph + outer orbit</span><input id="motionToggle" type="checkbox" checked></label>
        ...
        <div class="settings-section">
          ...
          <label class="settings-row"><span>Connected spacing <output id="innerSpacingValue">125 px</output></span><input id="innerSpacing" type="range" min="60" max="260" value="125"></label>
          <label class="settings-row"><span>Outer shell <output id="outerPullValue">0</output></span><input id="outerPull" type="range" min="-10" max="150" value="0"></label>
          <label class="settings-row"><span>Physics strength <output id="physicsStrengthValue">100%</output></span><input id="physicsStrength" type="range" min="25" max="175" value="100"></label>
          <label class="settings-row"><span>Rotation speed <output id="rotationSpeedValue">100%</output></span><input id="rotationSpeed" type="range" min="0" max="200" value="100"></label>
        </div>
  ...
  <script type="module">
    ...
    let outerOrbitElapsed = 0;
    let outerOrbitLastFrame = 0;
    ...
    function stopOuterOrbit() {
      ...
      outerOrbitFrame = undefined;
    }

    function startOuterOrbit() {
      stopOuterOrbit();
      if (!uiState.motionEnabled || !outerOrbitLayout) return;
      outerOrbitLastFrame = performance.now();
      const renderOuterOrbit = (now) => {
        if (!uiState.motionEnabled || !outerOrbitLayout) return;
        outerOrbitElapsed += now - outerOrbitLastFrame;
        outerOrbitLastFrame = now;
        positionOuterOrbit(outerOrbitElapsed);
        outerOrbitFrame = window.requestAnimationFrame(renderOuterOrbit);
      };
      outerOrbitFrame = window.requestAnimationFrame(renderOuterOrbit);
    }

    function outerRingRadius(clusterCount) {
      const spacingRadius = 250 + clusterCount * 10;
      const shellRadius = 460 + uiState.outerPull * 8;
      return Math.max(spacingRadius, shellRadius);
    }

    function positionOuterOrbit(elapsed = 0) {
      if (!outerOrbitLayout) return;
      const innerAngle = elapsed * 0.00008 * uiState.rotationSpeed;
      const outerAngle = elapsed * -0.00008 * uiState.rotationSpeed;
      const horizontalStretch = 1 + (uiState.horizontalVibe / 100) * 4;
      const center = outerOrbitLayout.center;
      for (const node of outerOrbitLayout.coreNodes) {
        const angle = node.angle + innerAngle;
        network.moveNode(node.id,
          center.x + node.radius * horizontalStretch * Math.cos(angle),
          center.y + node.radius * Math.sin(angle));
      }
      for (const cluster of outerOrbitLayout.outerClusters) {
        const angle = cluster.angle + outerAngle;
        const clusterX = center.x + outerOrbitLayout.radius * horizontalStretch * Math.cos(angle);
        const clusterY = center.y + outerOrbitLayout.radius * Math.sin(angle);
    ...
    function captureOuterOrbitLayout() {
      ...
      const outerThreshold = radii[Math.floor(radii.length * 0.72)] || 200;
      ...
      outerNodeIds.forEach(id => visNodes.update({ id, fixed: { x: true, y: true } }));
      visEdges.forEach(edge => {
        const isOuterClusterLink = outerNodeIds.has(edge.from) && outerNodeIds.has(edge.to);
        visEdges.update({ id: edge.id, smooth: isOuterClusterLink ? false : { type: 'continuous' } });
      });
      const coreNodes = nodes.filter(node => !outerNodeIds.has(node.id));
      coreNodes.forEach(node => visNodes.update({ id: node.id, fixed: { x: true, y: true } }));
      outerOrbitLayout = {
        center,
        coreNodes,
        outerClusters,
        radius: outerRingRadius(outerClusters.length)
      };
      outerOrbitElapsed = 0;
      positionOuterOrbit();
      network.stopSimulation();
      if (uiState.motionEnabled) startOuterOrbit();
      if (needsInitialFit) {
    ...
    function reheatLayout() {
      if (horizontalVibeFrame) window.cancelAnimationFrame(horizontalVibeFrame);
      stopOuterOrbit();
      visNodes.forEach(node => visNodes.update({ id: node.id, fixed: false }));
      if (uiState.motionEnabled) {
        network.setOptions({
          physics: {
            enabled: true,
            forceAtlas2Based: {
              gravitationalConstant: -60 * uiState.physicsStrength,
              springLength: uiState.innerSpacing,
              centralGravity: 0.004,
              springConstant: 0.07 * uiState.physicsStrength,
              avoidOverlap: 1
            }
          }
        });
    ...
    let resumeOrbitTimer;
    function pauseOrbitForPointer() {
      if (!uiState.motionEnabled) return;
      stopOuterOrbit();
      window.clearTimeout(resumeOrbitTimer);
    }

    function resumeOrbitAfterPointer() {
      if (!uiState.motionEnabled) return;
      window.clearTimeout(resumeOrbitTimer);
      resumeOrbitTimer = window.setTimeout(startOuterOrbit, 180);
    }

    container.addEventListener('pointerdown', pauseOrbitForPointer);
    container.addEventListener('pointerup', resumeOrbitAfterPointer);
    container.addEventListener('pointercancel', resumeOrbitAfterPointer);
    ...
    function resetGraph() {
      ...
      document.getElementById('innerSpacing').value = '125';
      document.getElementById('innerSpacingValue').textContent = '125 px';
    ...
    network.on('click', (params) => {
      ...
      if (params.nodes.length === 0) resumeOrbitAfterPointer();
    });
    ...
    document.getElementById('outerPull').addEventListener('input', (event) => {
      ...
      if (outerOrbitLayout) {
        outerOrbitLayout.radius = outerRingRadius(outerOrbitLayout.outerClusters.length);
        positionOuterOrbit(outerOrbitElapsed);
      }
    });
