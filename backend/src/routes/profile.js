...
import { sendVerificationCode, verifyCode, normalizeSriLankanPhone } from '../services/integrations.js';
...
router.post('/phone/send', otpSendLimiter, async (req, res) => {
  ...
  if (!phone) return res.status(400).json({ error: 'Enter a valid Sri Lankan mobile number, e.g. 0771575701' });
  try {
...
router.post('/phone/verify', otpVerifyLimiter, async (req, res) => {
  const phone = normalizeSriLankanPhone(req.body.phone);
  if (!phone || !/^\d{4,10}$/.test(String(req.body.code || ''))) return res.status(400).json({ error: 'valid phone and OTP code are required' });
  try {
