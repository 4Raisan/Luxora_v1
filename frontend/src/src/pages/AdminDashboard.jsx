...
const complaintReporterType = (complaint) => {
  const role = complaint.reporterType || complaint.fromRole || complaint.role || complaint.user?.role || 'Customer'
  return String(role).toLowerCase() === 'provider' ? 'Provider' : 'Customer'
}
...
export default function AdminDashboard() {
  const [adminUser] = useState(() => {
    try {
      ...
      if (u) {
        ...
        if (parsed.name) return { ...parsed, name: 'Super Admin', title: 'Super Admin' }
      }
    } catch (_) {}
    return { name: 'Super Admin', title: 'Super Admin', email: 'deshan@luxora.com' }
  })
  ...
  const [complaintView, setComplaintView] = useState('Customer')
  ...
  useEffect(() => {
    const syncSupportComplaints = () => {
      try {
        const saved = localStorage.getItem('luxora_support_complaints')
        const submittedComplaints = saved ? JSON.parse(saved) : []
        if (!Array.isArray(submittedComplaints)) return
        setComplaints((current) => {
          const currentIds = new Set(current.map((complaint) => complaint.id))
          return [...submittedComplaints.filter((complaint) => !currentIds.has(complaint.id)), ...current]
        })
      } catch (_) {}
    }

    window.addEventListener('storage', syncSupportComplaints)
    window.addEventListener('luxora_support_complaints_updated', syncSupportComplaints)
    return () => {
      window.removeEventListener('storage', syncSupportComplaints)
      window.removeEventListener('luxora_support_complaints_updated', syncSupportComplaints)
    }
  }, [])
  ...
  const loadAll = async () => {
    try {
      ...
      try {
        const saved = localStorage.getItem('luxora_support_complaints')
        submittedComplaints = saved ? JSON.parse(saved) : []
      } catch (_) {}
      const apiComplaints = Array.isArray(c) ? c : []
      const apiComplaintIds = new Set(apiComplaints.map((complaint) => complaint.id))
      setStats(s); setProviders(p); setBookings(b)
      setComplaints([
        ...(Array.isArray(submittedComplaints) ? submittedComplaints.filter((complaint) => !apiComplaintIds.has(complaint.id)) : []),
        ...apiComplaints
      ])
      setPromotions(pr)
    } catch (err) {
      ...
      customUsers = uniqueUsers(customUsers)
      try {
        localStorage.setItem('luxora_all_users', JSON.stringify(customUsers))
      } catch (_) {}
      ...
      // Saved users take priority over the fallback records. Matching by email
      // prevents the same account from appearing twice after a reload.
      setUsers(uniqueUsers([...customUsers, ...defaultUsersList]))
      ...
      setComplaints([
        ...(Array.isArray(submittedComplaints) ? submittedComplaints : []),
        ...defaultComplaints
      ])
      setPromotions([
  ...
  const handleKyc = async (id, status) => {
    ...
    if (target && status === 'approved') {
      setUsers(prev => {
        const existingUser = prev.find(u => u.email?.toLowerCase() === target.email?.toLowerCase())
        const providerUser = {
          ...(existingUser || {}),
          id: existingUser?.id || `USR-PRO-${target.id}`,
          name: target.name,
          email: target.email,
          role: 'Provider',
          category: target.category,
          registered: existingUser?.registered || new Date().toISOString().split('T')[0]
        }
        // Place an approved provider first so they appear at the top of the
        // Provider tab, while replacing any earlier record for that email.
        const updatedUsers = [providerUser, ...prev.filter(u => u.email?.toLowerCase() !== target.email?.toLowerCase())]
        try {
  ...
  const handleComplaintStatus = (id, newStatus) => {
    ...
    setComplaints(prev => {
      const updated = prev.map(c => c.id === id ? { ...c, status: newStatus, statusBg: bgMap[newStatus] || '#854d0e' } : c)
      try {
        const saved = localStorage.getItem('luxora_support_complaints')
        const submittedComplaints = saved ? JSON.parse(saved) : []
        if (Array.isArray(submittedComplaints)) {
          localStorage.setItem('luxora_support_complaints', JSON.stringify(
            submittedComplaints.map(c => c.id === id ? { ...c, status: newStatus, statusBg: bgMap[newStatus] || '#854d0e' } : c)
          ))
        }
      } catch (_) {}
      return updated
  ...
  return (
    <div className="ad-wrapper">
          ...
          <div className="ad-topbar__actions">
            ...
            >
              <div className="ad-user-avatar">
                {(adminUser.name || 'Super Admin').charAt(0)}
              </div>
              <div className="ad-user-info">
                <span className="ad-user-name">{adminUser.name || 'Super Admin'}</span>
        ...
        <div className="ad-content">
            ...
            <>
              ...
              <div className="ad-hero">
                ...
                <div className="ad-hero__content">
                  <p className="ad-hero__eyebrow">WELCOME BACK, {(adminUser.name || 'Super Admin').toUpperCase()}</p>
          ...
          {activeNav === 'users' && (
            <div className="ad-table-card" style={{ marginTop: 0 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
                <h3 className="ad-table-title" style={{ margin: 0 }}>USER MANAGEMENT DIRECTORY</h3>
                <button
              ...
              <div style={{ display: 'flex', gap: '0.6rem', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
                {['Customer', 'Provider', 'Admin'].map((role) => {
                  const count = users.filter((user) => user.role?.toLowerCase() === role.toLowerCase()).length
                  const selected = userRoleView === role
                  return (
                    <button
                      key={role}
                      type="button"
                      ...
                      style={{
                        background: selected ? 'var(--gold, #c9a84c)' : '#181818',
                        color: selected ? '#000' : '#ddd',
                        border: '1px solid ' + (selected ? 'var(--gold, #c9a84c)' : '#333'),
                        borderRadius: '7px',
              ...
              <p style={{ color: '#999', fontSize: '0.8rem', margin: '0 0 0.9rem' }}>
                Showing {userRoleView.toLowerCase()} accounts only.
              </p>
              <table className="ad-data-table">
                <thead>
                  <tr>
                ...
                <tbody>
                  {users.filter((u) => u.role?.toLowerCase() === userRoleView.toLowerCase()).map((u) => (
                    <tr key={u.id}>
                  ...
                  {users.filter((u) => u.role?.toLowerCase() === userRoleView.toLowerCase()).length === 0 && (
                    <tr>
                      <td colSpan="6" style={{ color: '#888', textAlign: 'center', padding: '1.5rem' }}>
                        No {userRoleView.toLowerCase()} accounts found.
                      </td>
                    </tr>
                  )}
                </tbody>
          ...
          {activeNav === 'complaints' && (
            <div className="ad-table-card" style={{ marginTop: 0 }}>
              ...
              <div style={{ display: 'flex', gap: '0.6rem', margin: '0.9rem 0 1.25rem', flexWrap: 'wrap' }}>
                {['Customer', 'Provider'].map((type) => {
                  const count = complaints.filter((complaint) => complaintReporterType(complaint) === type).length
                  const selected = complaintView === type
                  return (
                    <button
                      key={type}
                      type="button"
                      onClick={() => setComplaintView(type)}
                      style={{ background: selected ? 'var(--gold, #c9a84c)' : '#181818', border: `1px solid ${selected ? 'var(--gold, #c9a84c)' : '#333'}`, borderRadius: '7px', color: selected ? '#000' : '#ddd', cursor: 'pointer', fontSize: '0.78rem', fontWeight: 800, padding: '0.5rem 0.9rem' }}
                    >
              ...
              <table className="ad-data-table">
                ...
                <tbody>
                  {complaints.filter((complaint) => complaintReporterType(complaint) === complaintView).map((c) => (
                    <tr key={c.id}>
                      <td style={{ color: '#fff', fontWeight: 600 }}>{c.from}</td>
                      <td style={{ color: '#aaa', maxWidth: '360px' }}>
                        <div style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                          {c.detail?.replace(/^\[[^\]]+\]\s*/, '') || 'Service follow up'}
                        </div>
                      </td>
                      <td style={{ color: 'var(--gold, #c9a84c)', fontWeight: 700, fontSize: '0.78rem' }}>
                        {c.category || c.detail?.match(/^\[([^\]]+)\]/)?.[1] || 'General Inquiry'}
                      </td>
                      <td>
                        <span className="ad-badge-fill" style={{ background: c.statusBg, color: '#fff' }}>
                          {c.status}
                      ...
                      <td>
                        <button
                          type="button"
                          onClick={() => {
                            if (c.status !== 'RESOLVED') handleComplaintStatus(c.id, 'INVESTIGATING')
                            setSelectedComplaint(c)
                          }}
                          style={{ background: 'rgba(201, 168, 76, 0.12)', border: '1px solid var(--gold, #c9a84c)', borderRadius: '5px', color: 'var(--gold, #c9a84c)', cursor: 'pointer', fontSize: '0.72rem', fontWeight: 800, padding: '0.35rem 0.65rem', whiteSpace: 'nowrap' }}
                        >
                          VIEW DETAILS
                        </button>
                      </td>
                  ...
                  {complaints.filter((complaint) => complaintReporterType(complaint) === complaintView).length === 0 && (
                    <tr>
                      <td colSpan="5" style={{ color: '#888', padding: '1.5rem', textAlign: 'center' }}>
                        No {complaintView.toLowerCase()} complaints found.
                      </td>
                    </tr>
                  )}
                </tbody>
      ...
      {selectedComplaint && (
        <div className="ad-notif-overlay" onClick={() => setSelectedComplaint(null)}>
          <div className="ad-notif-modal" onClick={(event) => event.stopPropagation()} style={{ maxWidth: '520px' }}>
            <div className="ad-notif-modal__header">
              <div>
                <span className="ad-notif-modal__eyebrow">COMPLAINT DETAILS</span>
                <h3 className="ad-notif-modal__title">Customer Support Message</h3>
              </div>
              <button className="ad-notif-modal__close" onClick={() => setSelectedComplaint(null)}>✕</button>
            </div>
            <div style={{ display: 'grid', gap: '1rem', paddingTop: '0.75rem' }}>
              <div>
                <span style={{ color: '#888', display: 'block', fontSize: '0.7rem', fontWeight: 800, letterSpacing: '0.08em', marginBottom: '0.35rem' }}>MESSAGE TYPE</span>
                <span style={{ color: 'var(--gold, #c9a84c)', fontWeight: 700 }}>
                  {selectedComplaint.category || selectedComplaint.detail?.match(/^\[([^\]]+)\]/)?.[1] || 'General Inquiry'}
                </span>
              </div>
              <div>
                <span style={{ color: '#888', display: 'block', fontSize: '0.7rem', fontWeight: 800, letterSpacing: '0.08em', marginBottom: '0.35rem' }}>MESSAGE</span>
                <p style={{ color: '#eee', background: '#151515', border: '1px solid #2b2b2b', borderRadius: '8px', lineHeight: 1.65, margin: 0, padding: '1rem', whiteSpace: 'pre-wrap' }}>
                  {selectedComplaint.detail?.replace(/^\[[^\]]+\]\s*/, '') || 'Service follow up'}
                </p>
              </div>
              {selectedComplaint.status !== 'RESOLVED' && (
                <button
                  type="button"
                  onClick={() => {
                    handleComplaintStatus(selectedComplaint.id, 'RESOLVED')
                    setSelectedComplaint(null)
                  }}
                  style={{ background: '#4ade80', border: 'none', borderRadius: '7px', color: '#08210f', cursor: 'pointer', fontSize: '0.8rem', fontWeight: 900, padding: '0.7rem 1rem' }}
                >
                  DONE
                </button>
              )}
        ...
        <div className="ad-notif-overlay" onClick={() => setShowAdminProfileModal(false)}>
          <div className="ad-notif-modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '460px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem', padding: '1rem', background: '#161616', borderRadius: '12px', border: '1px solid var(--gold, #c9a84c)' }}>
              ...
              <div>
                <h4 style={{ color: '#fff', fontSize: '1.15rem', margin: 0, fontWeight: 800 }}>{adminUser.name || 'Super Admin'}</h4>
