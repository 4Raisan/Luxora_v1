...
:: 0. Preflight: Node.js must be installed
where node >nul 2>&1
if %errorlevel% neq 0 (
...
:: 0b. First run on a fresh PC: create backend/.env (gitignored) with dev defaults
if not exist "backend\.env" (
    echo [i] Creating backend\.env with local dev defaults...
    call :create_env
)
...
:: 1b. Build the frontend if it has not been built yet (fresh PC / fresh clone).
:: Without this, the backend serves only the API and the site shows "Cannot GET /".
if not exist "frontend\dist\index.html" (
    echo [1b/8] Frontend not built yet - building ^(first run only, takes a minute^)...
    pushd frontend
    call npm install
    if %errorlevel% neq 0 (
    ...
    call npm run build
    if %errorlevel% neq 0 (
        echo [WARN] Frontend build failed - the API will still run, but the website
        echo       will not be served. Fix errors above and re-run.
    ) else (
        echo [i] Frontend built successfully.
    )
    popd
    echo.
)
...
if %errorlevel% neq 0 (
    echo [ERROR] Prisma db push failed - no reachable PostgreSQL database.
    echo         Install ONE of these and run this file again:
...
:: ---------- Subroutine: write backend/.env with dev defaults ----------
:: Runs as a call so %JWTSEC% is expanded at execution time, not parse time.
:create_env
for /f "delims=" %%S in ('powershell -NoProfile -Command "[guid]::NewGuid().ToString('N') + [guid]::NewGuid().ToString('N')"') do set "JWTSEC=%%S"
(
    echo # Local development - auto-generated by start.bat on first run
    echo DATABASE_URL="postgresql://luxora_user:luxora_pass@localhost:5432/luxoradb?schema=public"
